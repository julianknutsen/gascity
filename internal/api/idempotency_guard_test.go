...
var exemptFromIdempotency = map[string]bool{
	...
	// Worker lifecycle verbs (cr-gdeav.5.4 draft) act on a bead the request
	// names, and each carries its own replay guard at the store, so an
	// Idempotency-Key would be dead weight: a repeated claim is the store's
	// holder-idempotent CAS, a repeated heartbeat just moves the lease instant
	// forward again, and a repeated close loses the AtomicConditionalCloser's
	// revision fence rather than duplicating anything
	// (internal/beads/beads.go:273-283).
